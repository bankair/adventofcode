...
def process(input)
  ...
  hypoallergenics = (input.keys.reduce(:+).uniq - ingredient_per_allergen.values)
  hypoallergenics.map { |hypoallergenic| input.keys.select { |ingredients| ingredients.include?(hypoallergenic) }.count }.sum
end
